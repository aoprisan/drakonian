...
All ritual content is static data, not fetched. The four Nightside arrays — `QLIPHOTH` (10 shells), `TUNNELS` (22 paths), `DEGREES`, `RITUALS` — are cross-referenced by id. **Invariants enforced by the smoke test:** exactly 10 shells with unique ids/orders, exactly 22 tree paths referencing valid shells, and every shell mapping to a real `degreeId` + `ritualId`. Keep these consistent when editing data.

`data/thelema.ts` holds the Thelema section: `THELEMA` (topic leaves), `FEASTS`, and `THELEMIC_RITES`. Those rites have **no `qliphaId`** — `Ritual.qliphaId` is optional and rites outside the Nightside carry a `home` link instead. `getRitual()` in `rituals.ts` spans both sets, so the one rite player serves every tradition.

### Thelemic reckoning (`src/sys/thelemic-date.ts`)
Pure offline almanac maths: `aeonYear()` counts docosades from the 1904 equinox ("Anno V:xii"), `sunLongitude()`/`moonLongitude()` give low-precision zodiac places (the Moon's is derived from the Sun's plus the phase angle from `lunar.ts`), and `reshStation()` picks the Liber Resh adoration matching the local clock. The Thelema view re-renders this strip on a 60s interval — cleared in `destroy()`.

### Build stamp & forced refresh (`src/sys/build.ts`, `refresh.ts`, `update-bus.ts`)
`vite.config.ts` `define`s `__BUILD_TIME__`, `__BUILD_COMMIT__` and `__APP_VERSION__`; `sys/build.ts` reads them behind `typeof` guards so the sources still bundle outside Vite (the smoke harness). `sys/refresh.ts` unregisters service workers and deletes every cache bucket, then reloads — it deliberately does **not** import `virtual:pwa-register`, which stays confined to `sys/update.ts`; the two talk through `sys/update-bus.ts`. The colophon card (`components/build-info.ts`, shown in the Mysteries view) surfaces the stamp and both controls.
...
### Chrome & styling (`src/components/nav.ts`, `src/styles/main.css`)
One stylesheet, no preprocessor. `:root` carries the palette plus a spacing ladder (`--s-1`…`--s-6`), surface gradients, a focus `--ring`, and safe-area insets. Shared primitives — `.page-head`/`.kicker`, `.leaf` (the ruled panel), `.card-grid`, `.section-title` — are what newer sections are built from; reach for them before inventing a layout. The nav is a single element that folds its link list into a drawer under 46rem (`.topnav.menu-open`), closing on route change, Escape and outside click.
